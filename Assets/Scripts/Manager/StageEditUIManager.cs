...
public class StageEditUIManager : BaseStageUIManager {
	...
	public override void ShowStageInformation(Stage stage) {
		...
		if (coinText)
			coinText.text = GameObject.FindGameObjectsWithTag(Tags.BLOCK).Where(block => block.GetComponent<BlockStatus>().blockType == Block.Coin).Count().ToString();
	}
	...
	public int IncreaseCoinCount(int num = 1) {
		coinText.text = (int.Parse(coinText.text) + num).ToString();
		return int.Parse(coinText.text);
	}

	public int DecreaseCoinCount(int num = 1) {
		coinText.text = (int.Parse(coinText.text) - num).ToString();
		return int.Parse(coinText.text);
	}
